...
import { Text, TouchableOpacity, StyleSheet, Image, View, Platform, ActivityIndicator } from 'react-native';
...
const FilePicker = (props) => {
    ...
    const [chosenItemsReset, setchosenItemsReset] = useState(false);
          ...
          await ImagePicker.showImagePicker({}, async response => {
            if (response.didCancel) {              
             ...
             const imageResponse = await new Promise((resolve, reject) => {
              ImageResizer.createResizedImage(uri,  672, 896, `${type.replace('image/', '')}`.toUpperCase() , 70)
              .then(response => {
                resolve(response);
              })
              .catch(err => {
              ...
              const s3key = await handlerSaveImage({uri: imageResponse.uri, type}, session);
